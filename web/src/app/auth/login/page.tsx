  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        // Verify backend-returned role matches the intended access
        // The frontend selector is only for login intent, not authorization
        const actualRole = response.data.user.role;

        if (actualRole === 'DOCTOR') {
          router.push('/doctor/dashboard');
        } else if (actualRole === 'SECRETARY') {
          ...
          setError('Access denied. Doctor or Secretary access required.');
          localStorage.removeItem('token');
          localStorage.removeItem('refreshToken');
          localStorage.removeItem('user');
          apiClient.clearToken();
      ...
      <div className="w-full max-w-[460px] flex flex-col items-center">
        ...
        <div className="text-center mb-7 w-full">
          <h1 className="text-[34px] font-black tracking-tight text-[#0D1829] mb-2 leading-tight">
            {role === 'DOCTOR' ? 'Doctor Portal' : 'Secretary Portal'}
          </h1>
        ...
        <div className="w-full bg-[#EAEFF5] p-1 rounded-full mb-7">
          <div className="flex gap-1">
            <button
              ...
              className={`flex-1 rounded-full py-2.5 px-4 text-center transition-all flex items-center justify-center gap-2 ${
                role === 'DOCTOR'
                  ? 'bg-white shadow-[0_1px_3px_rgba(0,0,0,0.06)] text-[#1A62CD] font-bold'
            ...
            >
              ...
              <span className="text-[15px] tracking-wide">Doctor</span>
            </button>
            <button
              type="button"
            ...
            >
              ...
              <span className="text-[15px] tracking-wide">Secretary</span>
            </button>
          </div>
        </div>
          ...
          <div className="space-y-2">
            ...
            <div className="relative rounded-2xl shadow-sm">
              ...
              <input
                ...
                placeholder={role === 'DOCTOR' ? 'doctor@example.com' : 'secretary@example.com'}
                required
